...
*/
RBTreeNode *makeRBTreeNode(char data, RBTreeNode *parent, RBTreeNode *leftchild, RBTreeNode *rightchild)
{
	RBTreeNode *newNode;
	if (data < 0)
		return (NULL);
	newNode = malloc(sizeof(RBTreeNode));
	if (!newNode)
		return (NULL);
	newNode->color = red;
	newNode->data = data;
	newNode->pParent = parent;
	newNode->pLeftChild = leftchild;
	newNode->pRightChild = rightchild;
	return (newNode);
}
...
RBTree *makeRBTree(void)
{
	...
	if (!newTree)
		return (NULL);
	Nil = makeRBTreeNode(' ', NULL, NULL, NULL);
	if (!Nil)
	{
		free(newTree);
		newTree = NULL;
...
int addNodeRB(RBTree *pTree, RBTreeNode element)
{
	...
	{	
		printf("ADD TO ROOT\n");
		newNode = makeRBTreeNode(element.data, pTree->nilNode, pTree->nilNode, pTree->nilNode);
		newNode->color = black;
...
static void addNodeRBrecursive(RBTree *T, RBTreeNode *N, int key)
{
	...
	newNode = makeRBTreeNode(key, N, T->nilNode, T->nilNode);
	if (key == N->data)
...
{
	RBTreeNode	*result;
	int 		N_color; // 삭제할 노드의 색을 저장할 자료이다.
